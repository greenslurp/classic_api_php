<?php
    ...
    function save_supporter_using_save($client, $cred) {
        // Change the first name to a date string.
        $first_name = date(DATE_RFC2822);
        $command = '/save';
        $method = "POST";
        $queries = [
            body => "&json=True"
                . "&object=supporter&key=" . $cred['supporter_KEY']
    ...
    function main() {
        ...
        print_r("Supporter was " . $data->First_Name . " " . $data->Last_Name . " " . $data -> Email . "\n");
        $data = save_supporter_using_save($client, $cred);
        $data = get_supporter($client, $cred);
